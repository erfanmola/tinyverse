...
function WebAudioTinySynthCore(target) {
	Object.assign(target, {
		...
		_guiUpdate: () => {
			if (this.canvas) {
				...
				if (this.song) {
					...
					this.ctx.beginPath();
					this.ctx.arc(
						80 + (this.playTick / this.maxTick) * 128,
						16,
						6,
						0,
						6.28,
		...
		click: (e) => {
			...
			if (pos.x < 40 && this.song) {
				if (this.playing) this.stopMIDI();
				else if (this.song) this.playMIDI();
			}
			if (
				pos.x >= 215 &&
				pos.x < 243 &&
    ...
    */
					...
					if (this.playing && this.song.ev.length > 0) {
						...
						while (this.actx.currentTime + this.preroll > this.playTime) {
							if (e.m[0] == 0xff51) {
								...
								this.tick2Time =
									(4 * 60) / this.song.tempo / this.song.timebase;
							} else this.send(e.m, this.playTime);
							++this.playIndex;
		...
		setMasterVol: (v) => {
			if (v != undefined) this.masterVol = v;
			if (this.out) this.out.gain.value = this.masterVol;
		},
		setReverbLev: (v) => {
			if (v != undefined) this.reverbLev = v;
			var r = parseFloat(this.reverbLev);
			if (this.rev && !isNaN(r)) this.rev.gain.value = r * 8;
		},
		setLoop: (f) => {
			this.loop = f;
		},
		...
		locateMIDI: (tick) => {
			...
			for (i = 0; i < this.song.ev.length && tick > this.song.ev[i].t; ++i) {
				...
				if (m.m[0] == 0xff51) this.song.tempo = m.m[1];
			}
			if (!this.song.ev[i]) {
				this.playIndex = 0;
				this.playTick = this.maxTick;
		...
		getTimbreName: (m, n) => {
			if (m == 0) return this.program[n].name;
			else return this.drummap[n - 35].name;
		},
		loadMIDIfromSrc: () => {
		...
		stopMIDI: () => {
			this.playing = 0;
			for (var i = 0; i < 16; ++i) this.allSoundOff(i);
		},
		playMIDI: () => {
			if (!this.song) return;
					...
					case 0xf0:
						switch (v) {
							...
							case 0xf7:
								...
								datastart = 1 + datalen;
								var exd = Array.from(
									s.slice(i + datastart, i + datastart + len),
								);
								exd.unshift(0xf0);
    ...
    */
		...
		setQuality: (q) => {
			if (q != undefined) this.quality = q;
			for (let i = 0; i < 128; ++i) this.setTimbre(0, i, this.program0[i]);
			...
			if (this.quality) {
				...
				for (let i = 0; i < this.drummap.length; ++i) {
					if (this.drummap1[i]) this.setTimbre(1, i + 35, this.drummap1[i]);
				}
			}
		},
		setTimbre: (m, n, p) => {
			...
			if (m && n >= 35 && n <= 81) this.drummap[n - 35].p = filldef(p);
			if (m == 0 && n >= 0 && n <= 127) this.program[n].p = filldef(p);
		},
		_pruneNote: (nt) => {
			...
			const f =
				...
				Math.pow(
					...
					(n -
						...
						this.tuningC[ch] +
						(this.masterTuningF +
							this.tuningF[ch] / 8192 +
							this.scaleTuning[ch][n % 12])) /
						12,
				);
			...
			for (let i = 0; i < p.length; ++i) {
				...
				switch (pn.w[0]) {
					...
					default:
						...
						if (pn.w[0] == "w") o[i].setPeriodicWave(this.wave[pn.w]);
						else o[i].type = pn.w;
						if (o[i].detune) {
							this.chmod[ch].connect(o[i].detune);
							o[i].detune.value = this.bend[ch];
		...
		setChVol: (ch, v, t) => {
			...
			this.chvol[ch].gain.setValueAtTime(
				this.vol[ch] * this.ex[ch],
				this._tsConv(t),
			);
		},
		setPan: (ch, v, t) => {
		...
		setProgram: (ch, v) => {
			if (this.debug) console.log("Pg(" + ch + ")=" + v);
			this.pg[ch] = v;
		},
		_tsConv: (t) => {
			if (t == undefined || t <= 0) {
				...
				if (this.tsmode) t = t * 0.001 - this.tsdiff;
			}
			return t;
		},
		setBend: (ch, v, t) => {
		...
		noteOn: (ch, n, v, t) => {
			...
			t = this._tsConv(t);
			if (this.rhythm[ch]) {
				if (n >= 35 && n <= 81) this._note(t, ch, n, v, this.drummap[n - 35].p);
				return;
			}
					...
					switch (msg[1]) {
						...
						case 38 /* data entry lsb */:
							switch (this.rpnidx[ch]) {
								...
								case 1:
									this.tuningF[ch] =
										(((this.tuningF[ch] + 0x2000) & 0x3f80) | msg[2]) - 0x2000;
									break;
								case 2:
									break;
							}
					...
					if (msg[0] == 0xf0) {
						...
						if (
							...
							if ((msg[6] & 0xf0) == 0x10 && msg.length == 11) {
								...
								if (msg[7] == 0x15) {
									this.rhythm[c] = msg[8];
								} else if (msg[7] >= 0x40 && msg[7] <= 0x4b) {
									// Scale Tuning
									this.scaleTuning[c][msg[7] - 0x40] = (msg[8] - 0x40) / 100;
								}
							} else if (msg[6] == 0) {
								if (msg[7] == 0 && msg.length == 14) {
									// Master Tuning
									this.masterTuningF =
										(msg[8] * 0x1000 +
											msg[9] * 0x100 +
		...
		setAudioContext: (actx, dest) => {
			...
			this.dest = dest;
			if (!dest) this.dest = actx.destination;
			this.tsdiff = performance.now() * 0.001 - this.actx.currentTime;
			if (this.debug) console.log("TSDiff:" + this.tsdiff);
...
class WebAudioTinySynth {
	constructor(opt) {
		...
		this.setQuality(1);
		if (opt) {
			if (opt.useReverb != undefined) this.useReverb = opt.useReverb;
			if (opt.quality != undefined) this.setQuality(opt.quality);
			if (opt.voices != undefined) this.setVoices(opt.voices);
		}
		this.init();
	}
}
...
(async () => {
	...
	fetch("/interstellar.mid").then(async (response) => {
